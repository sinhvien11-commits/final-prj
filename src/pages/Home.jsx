...
export default function Home() {
  ...
  const waitText = avgWaitMin ? `~${avgWaitMin}${t('units.minShort')}` : '—'
      ...
      <div className="pt-16 pb-20 px-4 flex flex-col gap-4">
        ...
        <div className="bg-surface border border-surface-container-high rounded-[20px] overflow-hidden mt-3">
          <div className="relative h-36 bg-gradient-to-br from-surface-container to-background flex items-center justify-center">
            ...
            <div className="absolute inset-0 flex items-end p-4">
              <Badge variant="live">{t('home.statusOpen')}</Badge>
            </div>
          </div>
          ...
          <div className="p-4 flex flex-col gap-3">
            <div>
              ...
              <p className="text-secondary text-xs mt-0.5">{t('home.tagline')}</p>
            </div>

            <div className="flex flex-col gap-1.5">
              <div className="flex items-center justify-between text-xs">
                <span className="text-secondary">{t('home.occupancy')}</span>
            ...
            <div className="grid grid-cols-3 gap-2 pt-1">
              <StatPill icon="wifi" label={t('home.statPing')} value="4ms" />
              <StatPill icon="people" label={t('home.statPlaying')} value="42/48" />
              <StatPill icon="timer" label={t('home.statWaiting')} value={waitText} />
            </div>
          </div>
        </div>
        ...
        <div className="bg-surface border border-surface-container-high rounded-xl p-4 flex flex-col gap-3">
          <div className="flex items-center justify-between">
            ...
            {activeOrders > 0 && <Badge variant="live">{t('home.ordersBadge', { count: activeOrders })}</Badge>}
          </div>

          {error && <p className="text-error text-xs">{t(error)}</p>}

          <div className="grid grid-cols-2 gap-3">
            <div className="bg-surface-container rounded-xl p-3 text-center">
              <p className="text-secondary text-xs uppercase tracking-wider">{t('home.processingOrders')}</p>
              <p className="text-primary font-display font-black text-2xl mt-1">{activeOrders}</p>
            </div>
            <div className="bg-surface-container rounded-xl p-3 text-center">
              <p className="text-secondary text-xs uppercase tracking-wider">{t('home.avgWait')}</p>
